...
export default class District extends Component {
    ...
    componentDidMount() {
        instance.get('/state_district_wise.json')
        .then((res) => {
...
console.log(disc[1])
            return(
                ...
                <Card body>
                  <CardTitle className="blue-text"><h4>{disc[0]}</h4></CardTitle>
                  <CardText className="text-center">
                    <span className="pink-text">{disc[1].notes}<br/></span>
                    <h5 className="orange-text">  Confirmed:{disc[1].confirmed}<br/></h5>
                     <h5 className="green-text"> Recovered:{disc[1].recovered}<br/></h5>
                     <h5 className="red-text"> Active: {disc[1].active}<br/></h5>
